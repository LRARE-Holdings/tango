...
function Pill({
  ...
  return (
    <span className="inline-flex items-center rounded-full border px-3 py-1 text-xs tracking-wide" style={style as any}>
      {children}
    </span>
  );
}
...
function Button({
  ...
  const cls =
    variant === "primary" ? `${base} hover:opacity-90` : `${base} border hover:opacity-80`;
...
export default function AccountClient() {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 gap-5 md:grid-cols-3">
          ...
          <div>
            ...
            <div className="mt-2 text-xs" style={{ color: "var(--muted)" }}>
              {renewal ? `Renews: ${formatDate(renewal)}` : "Upgrade anytime."}
            </div>
          </div>
      ...
      <Card
        title="Security"
        subtitle="Password, providers, and session controls live here."
        right={
          <Link
            href="/auth"
            className="focus-ring inline-flex items-center justify-center px-5 py-2.5 text-sm font-medium border transition hover:opacity-80"
      ...
      >
        <div className="flex items-center justify-between gap-4 flex-col sm:flex-row">
          <div className="text-sm leading-relaxed" style={{ color: "var(--muted)" }}>
            Change your password, reset access, or update sign-in methods.
          </div>
          <Button variant="danger" onClick={signOut}>
            Sign out
          </Button>
