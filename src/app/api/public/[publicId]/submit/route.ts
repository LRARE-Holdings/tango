...
export async function POST(
  ...
  const time_on_page_seconds = Math.max(0, Number(body?.time_on_page_seconds ?? 0));
  ...
  if (!doc) {
    return NextResponse.json({ error: "Not found", publicId }, { status: 404 });
  }
  ...
  const { data: recipient, error: recErr } = await admin
    .from("recipients")
    .insert({ document_id: doc.id, name, email })
    .select("id")
    .single();
