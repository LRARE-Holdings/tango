...
export async function DELETE(
  ...
  ctx: {
    params:
      | Promise<{ id: string; stackId: string; userId: string }>
      | { id: string; stackId: string; userId: string };
  }
) {
  try {
    const { id: workspaceIdentifier, stackId, userId: targetUserId } = (await ctx.params) as {
      id: string;
      stackId: string;
      userId: string;
    };
