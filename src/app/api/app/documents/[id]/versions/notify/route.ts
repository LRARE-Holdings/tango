...
function buildVersionEmail(input: {
  title: string;
  versionLabel: string;
  shareUrl: string;
  ...
  <div style="margin:0;padding:24px;background:#f7f7f8;font-family:ui-sans-serif,system-ui,-apple-system,Segoe UI,Roboto,Arial,sans-serif;color:#111;">
    <div style="max-width:560px;margin:0 auto;background:#fff;border:1px solid #e5e5e5;border-radius:14px;padding:24px;">
      ...
      <h1 style="margin:10px 0 0;font-size:20px;line-height:1.3;">Updated document version available</h1>
      <p style="margin:14px 0 0;color:#333;font-size:14px;line-height:1.6;">
        A new version has been published for <strong>${safeTitle}</strong> (v${escapeHtml(input.versionLabel)}).
      </p>
      <div style="margin-top:20px;">
        <a href="${safeUrl}" style="display:inline-block;background:#111;color:#fff;text-decoration:none;padding:11px 16px;border-radius:999px;font-size:14px;font-weight:600;">Review latest version</a>
      </div>
  ...
  const text = `Updated document version available

${input.title} now has a new version (v${input.versionLabel}).
Review latest version: ${input.shareUrl}
...
async function resolveDocumentForUser(documentId: string, userId: string) {
  ...
  const { data: docData, error: docErr } = await supabase
    .from("documents")
    .select("id,title,public_id,workspace_id,owner_id,password_enabled,password_hash,version_count")
    .eq("id", documentId)
    .maybeSingle();
  ...
  if (role === "owner" || role === "admin") return { allowed: true as const, doc };
  if (doc.owner_id === userId) return { allowed: true as const, doc };
  ...
  if (!respErr && responsibility) return { allowed: true as const, doc };
...
export async function POST(
  ...
  try {
    ...
    const versionLabelRaw = String(body?.version_label ?? "").trim();
    const versionNumberRaw = Number(body?.version_number ?? NaN);
    const defaultLabel = String(access.doc.version_count ?? 1);
    const versionLabel = versionLabelRaw
      ? (isValidVersionLabel(versionLabelRaw) ? versionLabelRaw : "")
      : (Number.isFinite(versionNumberRaw) && versionNumberRaw > 0 ? String(versionNumberRaw) : defaultLabel);
    if (!versionLabel) {
      return NextResponse.json({ error: "Version label is invalid. Use numeric format such as 1.2." }, { status: 400 });
    }
    ...
    const emailContent = buildVersionEmail({
    ...
    await Promise.all(
      emails.map(async (email) => {
        const result = await sendWithResend({
          to: email,
          subject: `New version available: ${String(access.doc.title ?? "Document")} (v${versionLabel})`,
